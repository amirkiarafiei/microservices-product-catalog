...
4.  **Observability (Implemented):**
    *   **Distributed Tracing:** OpenTelemetry SDK initialized in every service via `libs/common-python/src/common/tracing.py`. Spans exported to **Zipkin** using the Zipkin JSON exporter.
    *   **B3 Propagation:** Trace context propagated across HTTP boundaries using B3 headers (`X-B3-TraceId`, `X-B3-SpanId`, `X-B3-Sampled`). The API Gateway injects these headers when proxying requests.
    *   **Messaging Tracing:** RabbitMQ messages carry B3 headers for trace continuity across async boundaries. PRODUCER spans created on publish, CONSUMER spans created on consumption.
    *   **Auto-Instrumentation:** FastAPI, HTTPX, and aio-pika instrumented automatically for span creation.
    *   **Correlation ID:** Every request carries a `X-Correlation-ID` header (generated by Gateway if missing). Logged in every JSON log entry.
    *   **Centralized Logging:** JSON logs to stdout → Logstash → Elasticsearch. Logs include `trace_id`, `span_id`, and `correlation_id` for cross-service correlation in **Kibana**.
